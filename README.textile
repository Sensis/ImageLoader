<a href='http://ci.novoda.com/job/ImageLoader/'><img src='http://ci.novoda.com/job/ImageLoader/badge/icon'></a>   
...
* UrlNetworkManager.retrieveImage now throws a ImageNotFoundException when a Throwable is caught. This ensures that the notFoundResourceId image is displayed in this error scenario.
* TODO: we had to comment out the following in the root pom.xml when building at Sensis. If we don't and we publish our artifacts to our repo, the reporting dependency cannot be resolved. It does not exist in Maven Central and we couldn't seem to access the novoda repo at - https://github.com/novoda/public-mvn-repo/raw/master/releases. 
<pre>
  <parent>
...
* Due to the above commented out reporting dependency, building from the top level now requires android.maven.plugin.version to be set:
<pre> mvn clean install -Dandroid.maven.plugin.version=3.5.3 -Dandroid.sdk.path=...</pre>
